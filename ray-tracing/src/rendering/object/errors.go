...
// invalidReflectionCoefficientsError is the error where an Object is diffuse, specular or transmission reflections are
// not valid.
//
// Parameters:
...
//  An Error.
//
func invalidReflectionCoefficientsError(specularReflection, transmissionReflection, diffuseReflection float64) error {
	errorMessage := fmt.Sprintf("At least one of the reflections is smaller than 0 or they do not sum 1: " +
		"diffuse %v, specular %v, transmission %v.", diffuseReflection, specularReflection, transmissionReflection)
	return errors.New(errorMessage)
}
